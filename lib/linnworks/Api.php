...
class InventoryApi extends LinnworksApiBase{
    ...
    public function saveStockItem( $item ){
        ...
        // return $result['StockItems']['StockItem']['pkStockItemId']; // blocked by zakir - because of unmatched variable
        return $response['StockItems']['StockItem']['pkStockItemId'];
    ...
    //------------------------------------------------------------------------------------------------------------------
    //zakir
    public function getItemBySku($p_sku){
        return $response = $this->getStockItem(array('SKU'=>$p_sku, 'IsSetSKU'=>'true'));
        //return $response['StockItems']['StockItem'];//['pkStockItemId'];
        //return $sku;
    }
    //------------------------------------------------------------------------------------------------------------------
    //zakir -
    public function saveStockItemBySkuAndQty($p_sku,$p_qty){
        $response = $this->getStockItem(array('SKU'=>$p_sku, 'IsSetSKU'=>'true'));
        // $values = array_values($response);
        $_pkStockItemId = $response['StockItems']['StockItem']['pkStockItemId'];
        $_location='Default';
        //return $_pkStockItemId . ' -- qty -- ' . $p_qty . '  --  Location -- '.$_location;
        return $result = $this->changeStockLevel($_pkStockItemId, $_location, $p_qty);
        /*$_location='Default';
